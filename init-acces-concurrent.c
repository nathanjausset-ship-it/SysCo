...
void lecteur()
{
   ...
   // On a fini de lire, on ferme l'accés en lecture
   fermer(fd[0]);
}
...
void ecrivain()
{
   ...
   // On a fini d'écrire, on ferme l'accés en écriture
   fermer(fd[1]);
}
...
void init()
{
  int r, l, e;
   ...
   // On ferme les deux cotés du tube
   fermer(fd[0]);
   fermer(fd[1]);
